...
def test_github_app_auth(url: str) -> None:
    ...
    with (
        ...
        patch("app.core.config.settings.GITHUB_APP_PRIVATE_KEY", "-----BEGIN RSA PRIVATE KEY-----\ntest\n-----END RSA PRIVATE KEY-----"),
...
def test_clone_multiple_repos_error():
    ...
    with patch("app.utils.git_utils.clone_or_update_repo") as mock_clone:
        ...
        mock_clone.side_effect = [os.path.join(tempfile.gettempdir(), "repo1"), Exception("Test error")]
        result = clone_multiple_repos(repo_urls)
